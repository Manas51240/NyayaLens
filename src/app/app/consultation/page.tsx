...
function ConsultationContent() {
  ...
  const generateMarkdown = (): string => {
    ...
    md += `**Subject Document:** ${activeDoc.title} (${activeDoc.fileName})\n`;
    md += `**Document Classification:** ${activeDoc.documentType}\n\n`;
    md += `---\n\n`;
    md += `### Scope & Responsible AI Notice\n`;
    md += `${brief.disclaimerNotice}\n\n`;
    md += `## 1. Document Purpose & Operational Context\n`;
    md += `${brief.documentPurpose || activeDoc.plainLanguageSummary}\n\n`;
    md += `## 2. Contracting Parties & Identified Jurisdiction\n`;
    brief.parties.forEach((p) => {
      md += `- **${p.name}** (${p.role})\n`;
    });
    md += `- **Governing Law / Forum:** ${brief.governingLawAndJurisdiction}\n\n`;
    md += `## 3. Key Business Commitments\n`;
    brief.keyBusinessTerms?.forEach((t) => {
      md += `- ${t}\n`;
    });
    activeDoc.obligations.slice(0, 5).forEach((ob) => {
      md += `- **${ob.party}:** ${ob.description} ${ob.deadline ? `(${ob.deadline})` : ''}\n`;
    });
    md += `\n## 4. Flagged Items for Counsel Examination (Risk Radar)\n`;
    activeDoc.risks.forEach((r) => {
      md += `### ${r.title} [Category: ${r.category} | Priority: ${r.severity.toUpperCase()}]\n`;
      md += `${r.explanation}\n`;
      if (r.quote) {
        md += `> "${r.quote}"\n`;
      }
  ...
  const handleDownloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Consultation_Memorandum_${activeDoc.fileName.replace(/\.[^.]+$/, '')}.md`;
    link.click();
