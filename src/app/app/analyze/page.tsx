    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 mt-1">
          Upload a contract or select a standard template to generate grounded explanations, a risk radar, and action steps.
        </p>
      </div>
      ...
      {/* Error Alert */}
      {error && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-lg text-xs sm:text-sm text-red-800 flex items-start gap-2.5">
      ...
      {loading && (
        <div className="p-6 bg-white border border-slate-200 rounded-lg shadow-2xs text-center space-y-4">
          <div className="w-10 h-10 border-3 border-slate-900 border-t-transparent rounded-full animate-spin mx-auto"></div>
          <div>
            <h3 className="text-sm font-semibold text-slate-900">
              Processing Untrusted Document Input
            </h3>
            <p className="text-xs text-slate-500 mt-1">{progressStep}</p>
          </div>
          <div className="max-w-md mx-auto bg-slate-100 rounded-full h-1.5 overflow-hidden">
      ...
      {!loading && activeTab === 'upload' && (
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={!file}
            className="w-full inline-flex items-center justify-center gap-2 py-3 px-4 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white font-semibold rounded-lg shadow-sm text-sm transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full text-xs sm:text-sm px-3.5 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-900"
            />
          </div>
      ...
      {/* Tab Content 3: Sample Contracts */}
